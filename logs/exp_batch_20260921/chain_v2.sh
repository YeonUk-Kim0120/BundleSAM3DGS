...
while read -r cfg ds s; do
  [ -z "$cfg" ] && continue
  GPU=$GPU bash $L/run_one_v2.sh $cfg $ds $s; rc=$?
  if [ $rc -eq 1 ]; then echo "[$(date '+%F %T')] RETRY $cfg $ds $s" >> $L/progress; GPU=$GPU bash $L/run_one_v2.sh $cfg $ds $s || echo "[$(date '+%F %T')] SKIPPED-AFTER-RETRY $cfg $ds $s" >> $L/progress; fi
done < $JOBS

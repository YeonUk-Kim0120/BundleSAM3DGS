...
def main() -> None:
    ...
    parser.add_argument("--gs_update_steps", type=int, default=500,
                        help="joint map+pose steps per cycle (v1: 500, v2: 350)")
    parser.add_argument("--gs_prealign_steps", type=int, default=0,
                        help="v2 pose-only pre-refinement steps for new keyframes "
                             "before append (0 = v1, original order)")
    parser.add_argument("--gs_pose_loss", choices=("total", "depth", "weighted", "normsum", "gate"), default="total",
                        help="stage-1 pose-gradient policy (map always trains on the total loss)")
    parser.add_argument("--gs_pose_depth_scale", type=float, default=25.0,
                        help="depth-gradient multiplier for --gs_pose_loss weighted")
    parser.add_argument("--gs_new_view_interval", type=int, default=0,
                        help="experiment D: every N-th joint step renders a keyframe that arrived "
                             "in this cycle (0 = v1 uniform sampling)")
    parser.add_argument("--gs_prior_mesh_npz", type=str, default=None,
                        help="override the SAM3D mesh npz (e.g. an oracle prior from experiments/make_oracle_prior.py)")
    parser.add_argument("--gs_prior_pose_json", type=str, default=None)
    parser.add_argument("--gs_prior_gaussian_ply", type=str, default=None)
    parser.add_argument("--gs_prior_skip_alignment", action="store_true",
                        help="use the pose json as-is (no online Sim(3) refinement)")
    parser.add_argument("--gs_prior_skip_color_transfer", action="store_true",
                        help="keep mesh vertex colors (no SAM3D gaussian color transfer)")
...
def configure_backend(args, cfg_nerf):
    ...
    for key, override in (("mesh_npz", args.gs_prior_mesh_npz), ("pose_json", args.gs_prior_pose_json),
                          ("gaussian_ply", args.gs_prior_gaussian_ply)):
        if override:
            if not os.path.isfile(override):
                raise FileNotFoundError(f"prior override missing: {override}")
            prior[key] = override
    prior_flags = {"skip_alignment": bool(args.gs_prior_skip_alignment),
                   "skip_color_transfer": bool(args.gs_prior_skip_color_transfer)}
    cfg_nerf["gaussian"] = {
        "runner_config": args.gs_runner_config,
        "device": "cuda:0",
        "initial_steps": int(args.gs_initial_steps),
        "update_steps": int(args.gs_update_steps),
        "prior": {**prior, "surfel_count": 20000, **prior_flags},
        "feedback": args.feedback,
        "pose_feedback": {"prealign_steps": int(args.gs_prealign_steps),
                          "new_view_interval": int(args.gs_new_view_interval),
                          "pose_loss": args.gs_pose_loss,
                          "pose_depth_scale": float(args.gs_pose_depth_scale)},
    }
    args._gs_manifest = {
        "runner_config": args.gs_runner_config,
        "initial_steps": int(args.gs_initial_steps),
        "update_steps": int(args.gs_update_steps),
        "prealign_steps": int(args.gs_prealign_steps),
        "new_view_interval": int(args.gs_new_view_interval),
        "pose_loss": args.gs_pose_loss,
        "pose_depth_scale": float(args.gs_pose_depth_scale),
        "prior": {**prior, **prior_flags},
    }

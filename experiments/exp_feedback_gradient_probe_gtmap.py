"""EXPERIMENT ⑤-4: is the pose gradient uninformative because the MAP is wrong, or because the LOSS is?

Copy of ``exp_feedback_gradient_probe.py`` (2026-09-11) with three additions; the original probe and the
main code are untouched.  Everything here is offline and read-only w.r.t. run directories.

1. ``--map-poses gt``: the map is built and refreshed with GT keyframe poses (poses locked, map only,
   same schedule as online: prior + first 5 keyframes for ``--initial-steps``, then ``--update-steps`` per
   keyframe).  ``--map-poses tracker`` reproduces the original probe (map from the tracker poses v1 saw).
   The newest keyframe is still probed at its TRACKER pose (pre-append), so the measurement is
   "does the loss point from the tracker pose toward GT when the reference map is right?".
2. ``--perturb-deg / --perturb-mm``: additionally probe at GT-pose + controlled world-side perturbations
   (rotation about the object centre, random axes with a fixed seed) so the direction quality is measured
   at known error sizes (basin size per term) — identical perturbations for both map variants.
3. Map verification (``--map-check-every``): Chamfer distance between the trained Gaussians (non-UNSEEN,
   opacity > 0.5, metric) and the GT mesh placed in the tracker's object frame (both directions, mm), and
   the rendered-depth residual of the newest keyframe at its GT pose (median |render − observed| mm inside
   the mask).  Without these the "GT map is a correct map" premise is unverified.
``--prior-variant sam3d|gtmesh``: the online SAM3D prior (aligned online) or the B oracle prior (GT mesh at GT
pose, alignment skipped, mesh colours).

  python3 experiments/exp_feedback_gradient_probe_gtmap.py --dataset ho3d \
    --video-dir datasets/HO3D_v3/evaluation/AP12 --run-dir outputs/gsfb_v1_ho3d_AP12_20260907 \
    --output-dir logs/gtmap_probe/AP12_gt --map-poses gt --probe-K $(seq 6 400) --device cuda:0
"""
...
def init_runner_like_online(cfg_path: Path, device: str, prior_paths: dict, frames: list[GaussianFrame],
                            cam_in_obs0: np.ndarray, initial_steps: int, prior_variant: str = "sam3d") -> GaussianRunner:
    """Mirror of bundlesdf.run_gaussian's first batch (prior + online Sim(3) + initialize_from_prior).

    ``prior_variant="gtmesh"``: B oracle prior — alignment skipped (pose json used as-is) and mesh vertex
    colours kept (no Gaussian-PLY colour transfer), exactly as the online ``skip_alignment`` /
    ``skip_color_transfer`` branches."""
    ...
    if prior_variant == "gtmesh":
        refined_pose, status = init_pose, "skipped (oracle GT-mesh prior, pose json as-is)"
    else:
...
# ----------------------------------------------------------------- map checks
def gt_mesh_in_tracker_frame(dataset: str, seq: str, gt: GroundTruth, n_samples: int = 200_000) -> np.ndarray:
    """GT mesh surface samples (metric) moved into the tracker's object frame (p_trk = align @ p_gt_obj)."""
    import trimesh
    from make_oracle_prior import load_gt_mesh

    mesh = load_gt_mesh(dataset, seq)
    pts, _ = trimesh.sample.sample_surface(mesh, n_samples, seed=0)
    pts = np.asarray(pts, dtype=np.float64)
    return pts @ gt.align[:3, :3].T + gt.align[:3, 3][None, :]


def map_quality(runner: GaussianRunner, mesh_pts_trk: np.ndarray, view, gt_c2w: np.ndarray,
                tracker_c2w: np.ndarray) -> dict:
    """Chamfer (Gaussians <-> GT mesh, mm) and rendered-depth residual (mm) of one view at GT / tracker pose."""
    from scipy.spatial import cKDTree

    with torch.no_grad():
        means = runner.splats["means"].detach().cpu().numpy().astype(np.float64)
        opac = torch.sigmoid(runner.splats["opacities"].detach()).cpu().numpy()
        keep = opac > 0.5
        if runner.lifecycle_fields is not None:
            keep &= (runner.lifecycle_fields.state.cpu().numpy() != STATE_UNSEEN)
    means_m = runner.normalization.metric_points(means[keep]).astype(np.float64)
    out = {"gaussians_used": int(keep.sum())}
    if len(means_m) > 10:
        d_g2m, _ = cKDTree(mesh_pts_trk).query(means_m)
        d_m2g, _ = cKDTree(means_m).query(mesh_pts_trk)
        out.update({"chamfer_g2m_median_mm": float(np.median(d_g2m) * 1000), "chamfer_g2m_mean_mm": float(d_g2m.mean() * 1000),
                    "chamfer_m2g_median_mm": float(np.median(d_m2g) * 1000), "chamfer_m2g_mean_mm": float(d_m2g.mean() * 1000),
                    "mesh_covered_frac_5mm": float((d_m2g < 0.005).mean())})
    device = runner.device
    for tag, c2w in (("gt", gt_c2w), ("tracker", tracker_c2w)):
        c2w_norm = torch.from_numpy(runner.normalization.normalize_c2w(c2w)).float().to(device)
        with torch.no_grad():
            sh_degree = min(runner.total_steps // int(runner.config["sh_degree_interval"]), int(runner.config["sh_degree"]))
            result = runner._rasterize(K=view.K.to(device)[None], c2w=c2w_norm[None], width=view.width, height=view.height,
                                       sh_degree=sh_degree, render_mode="RGB+ED", absgrad=False)
            rendered = result.colors[0, ..., 3] / runner.normalization.scale
            obs = view.depth.to(device)
            mask = view.mask.to(device)
            valid = mask & torch.isfinite(obs) & (obs >= float(runner.config["min_depth"])) & (obs <= float(runner.config["max_depth"])) \
                & (result.alpha[0, ..., 0] > float(runner.config["depth_alpha_threshold"]))
            if bool(valid.any()):
                diff = (rendered[valid] - obs[valid]).abs() * 1000.0
                out[f"depth_resid_{tag}_median_mm"] = float(diff.median()); out[f"depth_resid_{tag}_p90_mm"] = float(torch.quantile(diff, 0.9))
                out[f"depth_resid_{tag}_pixels"] = int(valid.sum())
            out[f"mask_covered_frac_{tag}"] = float(((result.alpha[0, ..., 0] > float(runner.config["depth_alpha_threshold"])) & mask).sum() / mask.sum().clamp_min(1))
    return out


def perturbed_pose(gt_c2w_metric: np.ndarray, norm, deg: float, mm: float, seed: int) -> np.ndarray:
    """GT pose with a world-side (object-centred, normalized frame) perturbation: random axis rotation of ``deg``
    and random direction translation of ``mm``.  Returned metric c2w."""
    rng = np.random.default_rng(seed)
    axis = rng.normal(size=3); axis /= np.linalg.norm(axis)
    tdir = rng.normal(size=3); tdir /= np.linalg.norm(tdir)
    delta = np.concatenate((tdir * (mm / 1000.0) * norm.scale, axis * math.radians(deg)))
    T = se3_exp_batch(torch.from_numpy(delta[None]).float())[0].numpy().astype(np.float64)
    gt_norm = norm.normalize_c2w(gt_c2w_metric).astype(np.float64)
    return norm.metric_c2w(T @ gt_norm)
...
def main() -> None:
    ...
    ap.add_argument("--prior-variant", choices=("sam3d", "gtmesh"), default="sam3d")
    ap.add_argument("--oracle-root", type=Path, default=REPO / "logs/oracle_priors_20260908")
    ap.add_argument("--map-poses", choices=("tracker", "gt"), default="gt")
    ...
    ap.add_argument("--perturb-deg", type=float, nargs="*", default=[1.0, 3.0, 5.0, 10.0])
    ap.add_argument("--perturb-mm", type=float, default=0.0, help="translation magnitude added to every perturbation")
    ap.add_argument("--perturb-samples", type=int, default=3, help="random axes per magnitude (fixed seeds)")
    ap.add_argument("--map-check-every", type=int, default=5, help="map quality every n-th probed cycle (0 = never)")
    args = ap.parse_args()

    seq = args.video_dir.name
    sub = "output_YCBInEOAT_sam2mask_mesh" if args.dataset == "ycb" else "output_HO3D_sam2mask_mesh"
    if args.prior_variant == "sam3d":
        ...
        prior_paths = {"mesh_npz": args.oracle_root / f"{seq}_gtmesh.npz", "pose_json": args.oracle_root / f"{seq}_gtmesh.json",
                       "gaussian_ply": None}
    for k, v in prior_paths.items():
        if v is not None and not Path(v).exists():
            raise FileNotFoundError(f"{k}: {v}")
    ...
    mesh_pts = gt_mesh_in_tracker_frame(args.dataset, seq, gt)
    print(f"[gtmap] {seq}: {len(cycles)} cycles, map poses = {args.map_poses}, prior = {args.prior_variant}, "
          f"probing K in [{min(probe_set)}..{max_K}], perturb deg {args.perturb_deg} x{args.perturb_samples} (+{args.perturb_mm} mm)",
          flush=True)

    def map_pose(fid: str, tracker_pose: np.ndarray) -> np.ndarray | None:
        if args.map_poses == "tracker":
            return tracker_pose.astype(np.float32)
        g = gt.gt_c2w(fid)
        return None if g is None else g.astype(np.float32)

    args.output_dir.mkdir(parents=True)
    with open(args.output_dir / "args.json", "w") as fh:
        json.dump({k: (str(v) if isinstance(v, Path) else v) for k, v in vars(args).items()}, fh, indent=1)
    t0 = time.time()
    rows: list[dict] = []
    quality: list[dict] = []
    ...
    for ci, cyc in enumerate(cycles):
        ...
        for j, fid in enumerate(new_ids):
            ...
            if pose is None:
                print(f"[gtmap] {fid}: no GT pose -> keyframe skipped for the map", flush=True)
                continue
            f = load_frame(args.run_dir, fid, K_cam, pose)
            frames.append(GaussianFrame(fid, f.rgb, f.depth, f.mask, f.K, f.c2w_cv).validated())
        if runner is None:
            first_pose = map_pose(cyc["ids"][0], cyc["before"][0])
            runner = init_runner_like_online(args.runner_config, args.device, prior_paths, frames,
                                             first_pose, args.initial_steps, args.prior_variant)
            consumed = K
            continue
        # refresh stored views with the chosen pose source (GT: no-op; tracker: the poses v1 saw this cycle)
        runner.refresh_view_poses({v.frame_id: map_pose(v.frame_id, cyc["before"][i]) for i, v in enumerate(runner.views)
                                   if map_pose(v.frame_id, cyc["before"][i]) is not None})
        do_probe = K in probe_set
        if do_probe:
            n_probed += 1
            for j, fid in enumerate(new_ids):
                gt_c2w = gt.gt_c2w(fid)
                if gt_c2w is None:
                    continue
                tracker_pose = cyc["before"][consumed + j]
                f = load_frame(args.run_dir, fid, K_cam, tracker_pose.astype(np.float32))
                view = runner._prepare_view(GaussianFrame(fid, f.rgb, f.depth, f.mask, f.K, f.c2w_cv).validated())
                for hide in args.hide:
                    r = probe_view(runner, view, tracker_pose, gt, fid, hide, args.step_deg, args.step_mm,
                                   args.pose_only_steps, args.pose_only_lr, args.pose_only_loss)
                    r.update({"seq": seq, "cycle": cyc["dir"], "K": K, "regime": "pre_append", "map_poses": args.map_poses})
                    rows.append(r)
                for deg in args.perturb_deg:
                    for si in range(args.perturb_samples):
                        seed = (int(fid) if fid.isdigit() else abs(hash(fid)) % 10**6) * 100 + int(deg * 10) + si
                        pp = perturbed_pose(gt_c2w, runner.normalization, deg, args.perturb_mm, seed)
                        r = probe_view(runner, view, pp, gt, fid, "none", args.step_deg, args.step_mm, 0, args.pose_only_lr)
                        r.update({"seq": seq, "cycle": cyc["dir"], "K": K, "regime": "perturb", "map_poses": args.map_poses,
                                  "perturb_deg": deg, "perturb_mm": args.perturb_mm, "sample": si})
                        rows.append(r)
                if args.map_check_every > 0 and (n_probed % args.map_check_every == 1 or args.map_check_every == 1):
                    q = map_quality(runner, mesh_pts, view, gt_c2w, tracker_pose)
                    q.update({"seq": seq, "cycle": cyc["dir"], "K": K, "frame_id": fid, "map_poses": args.map_poses,
                              "gaussians_total": runner.num_gaussians})
                    quality.append(q)
                    print(f"[gtmap] map quality K={K}: " + ", ".join(f"{k}={v:.2f}" if isinstance(v, float) else f"{k}={v}"
                                                                    for k, v in q.items() if "chamfer" in k or "resid" in k or "covered" in k), flush=True)
        stats = runner.update(frames, train_steps=args.update_steps) if frames else None
        consumed = K
        if do_probe:
            print(f"[gtmap] K={K} cycle {cyc['dir']} gaussians {stats.gaussians_after_train if stats else runner.num_gaussians} "
                  f"({time.time() - t0:.0f}s)", flush=True)
            with open(args.output_dir / "rows.json", "w") as fh:
                json.dump(rows, fh, indent=1)
            with open(args.output_dir / "map_quality.json", "w") as fh:
                json.dump(quality, fh, indent=1)
    with open(args.output_dir / "rows.json", "w") as fh:
        json.dump(rows, fh, indent=1)
    with open(args.output_dir / "map_quality.json", "w") as fh:
        json.dump(quality, fh, indent=1)
    summarize([r for r in rows if r["regime"] == "pre_append"], args.output_dir / "summary.txt")
    # perturbation summary
    with open(args.output_dir / "perturb_summary.txt", "w") as fh:
        for deg in args.perturb_deg:
            sel = [r for r in rows if r["regime"] == "perturb" and r["perturb_deg"] == deg]
            line = f"perturb {deg:4.1f} deg (n={len(sel)}): "
            for term in ("l1", "dssim", "depth", "total"):
                v = np.array([r["losses"][term]["cos_rot"] for r in sel if r["losses"].get(term)])
                if len(v):
                    line += f"{term} cos_rot {v.mean():+.2f} ({(v > 0).mean():.0%})  "

...
class RunnerCpuTest(unittest.TestCase):
    ...
    def test_config_validation(self):
        ...
        with self.assertRaises(ValueError):
            GaussianRunner({"device": "cpu", "pose_feedback": {"prealign_steps": -1}},
                           SceneNormalization(1.0, np.zeros(3)), device="cpu")
        with self.assertRaises(ValueError):
            GaussianRunner({"device": "cpu", "pose_feedback": {"new_view_interval": -1}},
                           SceneNormalization(1.0, np.zeros(3)), device="cpu")
        with self.assertRaises(ValueError):
            GaussianRunner({"device": "cpu", "pose_feedback": {"pose_loss": "rgb"}},
                           SceneNormalization(1.0, np.zeros(3)), device="cpu")
...
class PoseFeedbackGpuTest(unittest.TestCase):
    ...
    def _check(self, renderer):
        ...
        record = runner.prealign_views([view], steps=40, lr=0.02)
        self.assertIsNotNone(record)
        self.assertGreater(record["max_rot_deg"], 0.0)
        residual = rotation_angle_deg(
            view.c2w_normalized[:3, :3].numpy().T @ clean[:3, :3].numpy()
        )
        self.assertLess(residual, 2.0, f"{renderer}: pose-only refinement did not reduce a 2 deg error ({residual:.3f})")
        n_log = len(runner.feedback_log)
        first_loss, final_loss = runner.train(5, optimize_poses=True)
        ...
        # v2 path through update(): pre-alignment record + joint-training record.
        runner.config["pose_feedback"]["prealign_steps"] = 5
        stats = runner.update([make_plane_frame("c", 0.06)], train_steps=3)
        self.assertEqual(stats.train_steps, 3)
        self.assertEqual(len(runner.feedback_log), n_log + 3)
        self.assertTrue(runner.feedback_log[-2]["event"].startswith("prealign_"))
        self.assertTrue(runner.feedback_log[-1]["event"].startswith("train_"))
        self.assertEqual(runner.view_poses_metric().shape, (3, 4, 4))
        # experiment D: quota sampling of the cycle's new keyframes.
        runner.config["pose_feedback"]["prealign_steps"] = 0
        runner.config["pose_feedback"]["new_view_interval"] = 5
        runner.update([make_plane_frame("d", 0.09)], train_steps=50)
        record = runner.feedback_log[-1]
        self.assertEqual(record["new_view_interval"], 5)
        self.assertEqual(len(record["new_view_samples"]), 1)
        self.assertGreaterEqual(record["new_view_samples"][0], 10)   # 50 / 5 forced draws
        self.assertEqual(sum(runner._view_sample_counts), 50)
        runner.config["pose_feedback"]["new_view_interval"] = 0
        runner.update([make_plane_frame("e", 0.12)], train_steps=50)
        self.assertEqual(runner.feedback_log[-1]["new_view_interval"], 0)
        self.assertEqual(sum(runner._view_sample_counts), 50)

    def test_prealign_and_joint_training_2dgs(self):
        self._check("2dgs")

    def test_pose_gradient_policies(self):
        # stage 1 (E1/E2/E3/F): the map loss is untouched; only the pose-delta gradient changes.
        runner = self._fitted_runner("2dgs")
        fb = runner.config["pose_feedback"]
        depth_weight = runner.config["depth_loss_weight"]
        fb["new_view_interval"] = 1          # render the movable view (index 1) every step; view 0 is fixed

        def train(steps):
            runner.train(steps, optimize_poses=True, new_view_indices=[1])
            rec = runner.feedback_log[-1]
            return rec["max_rot_deg"] + rec["max_trans_mm"]

        fb["pose_loss"] = "depth"
        runner.config["depth_loss_weight"] = 0.0
        self.assertEqual(train(5), 0.0)               # no depth term -> depth policy cannot move
        runner.config["depth_loss_weight"] = depth_weight
        self.assertGreater(train(5), 0.0)
        fb["pose_loss"] = "weighted"
        self.assertGreater(train(5), 0.0)
        fb["pose_loss"] = "normsum"
        self.assertGreater(train(5), 0.0)
        fb["pose_loss"] = "gate"
        train(20)
        rec = runner.feedback_log[-1]
        self.assertEqual(rec["pose_loss"], "gate")
        self.assertTrue(0 <= rec["gate_moved_rot_steps"] <= 20)
        self.assertTrue(0 <= rec["gate_moved_trans_steps"] <= 20)
        fb["pose_loss"] = "total"
        self.assertGreater(train(5), 0.0)

    def test_prealign_and_joint_training_3dgs(self):
        self._check("3dgs")
